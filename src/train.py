...
USAGE  (run from project root):
    ...
    python src/train.py --image_size 320         # larger images (your 4070 handles this)
...
WHAT HAPPENS:
    ...
    2.  Auto-detect your fastMRI data directory
...
EXPECTED TRAINING TIME  (RTX 4070, 120 slices, 50 epochs, 256×256):
    ~ 8–15 minutes total.  Plenty fast.
"""
...
DEFAULT_CONFIG = dict(
    # Data 
    data_path            = 'data/singlecoil_val',   # auto-detected if missing
    ...
    out_channels         = 1,
    init_features        = 32,       # 32 → 7.8M params  |  64 → 31M params
    ...
    # DataLoader 
    num_workers          = 2,        # set to 0 on Windows if issues
)
...
def find_data_path(hint: str) -> str:
    ...
    for c in candidates:
        if os.path.isdir(c) and any(f.endswith('.h5') for f in os.listdir(c)):
            return c
    # last resort: walk data/ looking for .h5
    for root, dirs, files in os.walk('data'):
        if any(f.endswith('.h5') for f in files):
            return root
    raise FileNotFoundError(
        "Could not find fastMRI .h5 files.\n"
        "Please pass --data_path <directory> or put .h5 files in data/"
    )
...
def main():
    ...
    for epoch in range(start_epoch, config['num_epochs']):
        ...
        if val_loss < best_val_loss:
            ...
            print(f"      NEW  →  best_model.pth  (val_loss={val_loss:.4f})")
